...
import { LayoutDashboard, Bell, Users, BarChart3, Settings, PanelLeftClose, PanelLeftOpen, Brain, Cpu, Database, FolderGit, FileSpreadsheet, UserCheck, User, ShieldAlert } from "lucide-react";
...
interface Props {
  ...
  isAdmin?:      boolean;  // show developer tools section
}

// Developer mode can also be enabled via env var
const DEV_MODE_ENV = import.meta.env.VITE_DEVELOPER_MODE === "true";
...
const AppSidebar = ({
  ...
  const showDevTools = isAdmin || DEV_MODE_ENV;

  // Public sidebar — visible to all authenticated users
  const publicNavItems = [
    { icon: LayoutDashboard, label: "Dashboard",      id: "overview"  },
    { icon: BarChart3,       label: "Analytics",      id: "analytics" },
    { icon: Database,        label: "Transactions",   id: "transactions" },
    { icon: Users,           label: "Accounts",       id: "accounts"  },
    { icon: FolderGit,       label: "Investigations", id: "investigations" },
    { icon: Bell,            label: "Alerts Center",  id: "alerts", badge: alertCount > 0 ? alertCount : null },
    { icon: Brain,           label: "AI Copilot",     id: "copilot" },
    { icon: Settings,        label: "Settings",       id: "settings"  },
    { icon: User,            label: "Profile",        id: "profile" },
  ];

  // Developer / internal tools — admin-only
  const devNavItems = [
    { icon: Brain,           label: "Training Center",  id: "training" },
    { icon: Cpu,             label: "GPU Monitor",      id: "gpu" },
    { icon: BarChart3,       label: "AI Benchmarks",    id: "copilot_benchmark" },
    { icon: FileSpreadsheet, label: "Reports Exporter", id: "reports" },
    { icon: UserCheck,       label: "Admin Panel",      id: "admin" },
  ];

  const renderNavItem = (item: { icon: any; label: string; id: string; badge?: number | null }) => {
    ...
    >
      ...
      <nav className="flex-1 px-2 mt-1 overflow-y-auto">
        ...
        {showDevTools && (
          <>
            {isOpen ? (
              <div style={{ display: "flex", alignItems: "center", gap: 6, margin: "12px 4px 6px", padding: "4px 8px", borderTop: "1px solid #1a1a1a" }}>
                <ShieldAlert size={10} style={{ color: "#cc0000", flexShrink: 0 }} />
                <span style={{ fontSize: 9, fontFamily: "monospace", color: "#cc0000", letterSpacing: "0.15em", fontWeight: 700 }}>DEV TOOLS</span>
              </div>
            ) : (
              <div style={{ margin: "12px 0 6px", borderTop: "1px solid #1a1a1a" }} />
            )}
